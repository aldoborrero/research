# Cl@ve AEAT - Reverse Engineering Report
...
│   │   ├── ClaveApi.java           # Retrofit interface (all API endpoints)
...
|----------|--------|---------|
| `wlpl/MOVI-P24H/ClaveStartingSv` | POST (form) | App initialization, returns URL whitelists/blacklists and menu config |
| `wlpl/MOVI-P24H/ClaveIsNifActivatedSv` | POST (form) | Check if a NIF is activated in Cl@ve |
| `wlpl/MOVI-P24H/ClaveRequestPinSv` | POST (form) | Request a Cl@ve PIN (returns pin + TTL) |
| `wlpl/MOVI-P24H/ClaveAuthenticateSv` | POST (form) | Authenticate (Cl@ve Móvil confirm) |
| `wlpl/MOVI-P24H/ClaveCancelAuthenticateSv` | POST (form) | Cancel an authentication request |
| `wlpl/MOVI-P24H/ClaveActivateAuthenticationSv` | POST (form) | Activate device authentication (hosted at www6 or www1) |
| `wlpl/MOVI-P24H/ClaveDesactivateAuthSv` | POST (form) | Deactivate device authentication |
| `wlpl/MOVI-P24H/ClaveCheckMyDataSv` | POST (form) | Check user account data |
| `wlpl/MOVI-P24H/ClaveRequestAllOperationsSv` | POST (form) | Request all pending operations |
| `wlpl/MOVI-P24H/ClaveMigrationSv` | POST (form) | Migrate from old Cl@ve PIN to new version |
| `wlpl/MOVI-P24H/ClaveSetFirebaseTokenSv` | POST (form) | Register push notification token |
...
| `wlpl/MOVI-P24H/ClaveRequestStateSv` | POST (form) | Get current request state (www12 or www1) |

### Cl@ve Móvil Endpoints

| Endpoint | Method | Purpose |
|----------|--------|---------|
| `wlpl/MOVI-P24H/ObtenerClaveMovil` (www12) | GET | Get Cl@ve Móvil activation page |
| `wlpl/MOVI-P24H/ValidarClaveMovil` (www12) | POST (form) | Validate Cl@ve Móvil token |
| `wlpl/MOVI-P24H/ObtenerClaveMovilSMS` (www12) | POST | Get Cl@ve Móvil SMS code |
...
| Endpoint | Method | Purpose |
|----------|--------|---------|
| `wlpl/MOVI-P24H/ClaveOperationsHistorySv` | POST (form) | Query operations history with filters |
...
| `wlpl/MOVI-JDIT/Acceso?ext=OFICINAS_CLAVE` | Cl@ve offices |
...
- **www12.agenciatributaria.gob.es** — Cl@ve Móvil, state checks, SMS validation
...
### Flow 1: Cl@ve PIN Request (Main Flow)

```
1. ClaveStartingSv  →  Initialize session (device_id, NIF, OS info)
                       Response: { status: "OK", respuesta: { listaBlancaUrls, menu } }

2. ClaveIsNifActivatedSv  →  Check if NIF is registered
                              Response: device activation status

3. ClaveRequestPinSv  →  Request a temporary PIN
...
### Flow 2: Cl@ve Móvil Authentication

```
1. ClaveStartingSv  →  Initialize
2. (Push notification received with authentication request)
3. ClaveAuthenticateSv  →  Confirm authentication
...
Or to cancel:
```
3. ClaveCancelAuthenticateSv  →  Reject authentication
```
...
Found in `KeyChainManager.getValueClavePin()`:

```
Algorithm: AES/GCM/NoPadding
Key: AndroidKeyStore "ClavePinAlias" (AES, 256-bit)
...
2. User enters NIF
3. ClaveStartingSv → registers device, gets session
4. ClaveActivateAuthenticationSv → activates push notifications
...
|-----------------|---------|
| `ClavePin` | Legacy encrypted credential storage |
| `ClaveAuthentication` | Authentication passwords (migrated) |
| `ClaveCertificados` | Software certificate storage |
...
Clave Gateway:               https://pasarela.clave.gob.es/
Clave Gateway SP:            https://pasarela.clave.gob.es/Proxy2/ServiceProvider
...
1. **No TOTP/HOTP**: The app does NOT generate OTPs locally. PINs are server-generated via `ClaveRequestPinSv` and returned with a TTL.
...
6. **Multiple auth levels**:
   ...
   - **Medium**: Cl@ve PIN (server-generated, device-bound)
   - **Strong**: Cl@ve Móvil (push notification + confirm)
