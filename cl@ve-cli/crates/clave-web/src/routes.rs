...
async fn activate(
    ...
    let nif = match clave_core::config::validate_nif(&body.nif) {
        Ok(n) => n,
        Err(e) => {
            return (
    ...
    match clave_core::auth::activate_device(&state.client, &nif, &device_id, &device_password)
        .await
    {
        Ok(session) => {
...
async fn request_pin(State(state): State<AppState>) -> impl IntoResponse {
    match clave_core::auth::request_pin(&state.client, &state.session).await {
...
async fn get_pending(State(state): State<AppState>) -> impl IntoResponse {
    match clave_core::auth::poll_pending_requests(&state.client, &state.session).await {
...
async fn confirm_request(
    ...
    match clave_core::auth::confirm_authentication(
        &state.client,
        &state.session,
        &request_id,
        &idp_code,
...
async fn reject_request(
    ...
    match clave_core::auth::reject_authentication(
        &state.client,
        &state.session,
        &request_id,
        &idp_code,
